...
Stages:
    ...
    04. Pretrain EEG-ViT (in_chans=9) on PhysioNet spectrograms
    05. EEG-ViT-only baseline using PhysioNet-pretrained weights
    06. Tri-branch adaptive fusion (CSP + Riemannian + ViT)
...
All outputs go to a timestamped run directory, e.g.::
    ...
    runs/2024-01-15_143022/
        results/
            ...
            ...
        checkpoints/
            vit_pretrained_physionet_eeg_vit.pt
        figures/
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--epochs", type=int, default=200, help="Max training epochs per fold")
    ...
    p.add_argument(
        ...
        "Defaults to <run-dir>/checkpoints/vit_pretrained_physionet_eeg_vit.pt",
    )
    p.add_argument(
        "--processed-dir",
        default=None,
...
def main() -> None:
    ...
    backbone_short = "eeg_vit" if args.backbone == "eeg_vit_tiny_patch8_64" else args.backbone
    ...
    plan = [
        ...
        (
            6,
            "Tri-branch adaptive fusion (CSP + Riemannian + ViT)",
            not args.skip_dual,
            uv
            + ["scripts/pipeline/stage_06_tri_branch.py"]
            + base
            + nn
            + cv
            + proc
