...
subjects and trains an EEG-ViT classifier.  Saves
...
Output::
    ...
    <run-dir>/checkpoints/vit_pretrained_physionet_eeg_vit.pt
...
def run_mae_pretraining(
    ...
    """Self-supervised masked patch pretraining for EEGViT."""
    import torch
    import torch.nn.functional as F
    from torch.utils.data import DataLoader

    from bci.models.eeg_vit import EEGViTWithMasking

    mae_model = EEGViTWithMasking(
    ...
    log.info(
        "Starting MAE pretraining: epochs=%d, lr=%.2e, mask_ratio=%.2f", epochs, lr, mask_ratio
    )
    for ep in range(1, epochs + 1):
        ...
        for imgs, _ in loader:
            ...
            with torch.amp.autocast("cuda", enabled=(use_amp and device.type == "cuda")):
                pred, target, _ = mae_model.forward_masked(imgs)
        ...
        log.info("  MAE epoch %d/%d: recon_loss=%.5f", ep, epochs, mean_loss)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=10,
        help="Masked autoencoder pretraining epochs before LP-FT (0 to disable)",
    )
    p.add_argument(
        "--mae-lr",
        type=float,
        default=1e-3,
        help="Learning rate for MAE pretraining",
    )
    p.add_argument(
        "--mae-mask-ratio",
        type=float,
    ...
    p.add_argument("--no-mixup", action="store_true", help="Disable mixup during LP-FT")
    p.add_argument(
        "--no-spec-augment",
        action="store_true",
        help="Disable SpecAugment during LP-FT",
    )
    p.add_argument("--force", action="store_true", help="Overwrite existing checkpoint/results")
...
def main() -> None:
    ...
    checkpoint_path = run_dir / "checkpoints" / "vit_pretrained_physionet_eeg_vit.pt"
    out_path = run_dir / "results" / "real_pretrain_physionet_vit.json"
    plots_dir = run_dir / "plots" / "pretrain"

    if checkpoint_path.exists() and out_path.exists() and not args.force:
    ...
    # Target image size: 64×64 matches EEG-ViT training resolution.
    ...
    # ── Model ──────────────────────────────────────────────────────────────
    # Use img_size=64: matches EEG-ViT default input size.
    model_config = ModelConfig(
        vit_model_name="eeg_vit_tiny_patch8_64",
        vit_pretrained=True,
        vit_drop_rate=0.1,
    ...
    if args.mae_epochs > 0:
        ...
        src_state = mae_model.state_dict()
        transfer_state = {
            k: v for k, v in src_state.items() if k in tgt_state and v.shape == tgt_state[k].shape
        }
        missing, unexpected = model.backbone.load_state_dict(transfer_state, strict=False)
        log.info(
            "Loaded MAE init into ViT backbone (%d tensors, missing=%d, unexpected=%d)",
            len(transfer_state),
            len(missing),
            len(unexpected),
        )
