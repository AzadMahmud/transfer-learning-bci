...
def load_baseline_c(path: Path, loso_path: Path | None = None) -> dict | None:
    """Load Baseline C (EEG-ViT-only) – within + optional LOSO JSON."""
    data = _load_json(path)
    if data is None:
        return None
...
def load_tri_branch(path: Path, loso_path: Path | None = None) -> dict | None:
    """Load tri-branch adaptive result."""
    data = _load_json(path)
    if data is None:
        return None
    ...
    return {
        "model": f"{data.get('model', 'TriBranchAdaptive')} [{data.get('fusion', 'adaptive')} ]",
        "strategy": data.get("strategy", "within_subject"),
        "within_acc": data.get("mean_accuracy"),
        "within_std": data.get("std_accuracy"),
        "within_kappa": data.get("mean_kappa"),
        ...
        "per_subject": data.get("per_subject", {}),
        "loso_per_subject": lo.get("per_subject", {}),
        "fusion": data.get("fusion", "adaptive"),
    }
...
def analyze_baseline_vs_dual(summary: dict) -> list[dict]:
    ...
    results = []
    dual_attn = (
        summary.get("dual_branch", {}).get("tri_adaptive")
        or summary.get("dual_branch", {}).get("attention_v2")
        or summary.get("dual_branch", {}).get("attention")
    )
    ...
    for name, bsl in baselines:
        ...
        attn_tag = dual_attn.get("fusion", "attention")
        results.append(
            {
                "comparison": f"DualBranch({attn_tag}) vs {name}",
...
def analyze_fusion_methods(summary: dict) -> dict:
    ...
    methods = ["tri_adaptive", "attention_v2", "attention", "gated"]
...
def analyze_per_subject_variance(summary: dict) -> dict:
    ...
    accs = list(ps.values())
    return {
        "model": f"DualBranch ({dual_attn.get('fusion', 'attention')})",
        "n_subjects": len(accs),
...
def plot_fusion_ablation(summary: dict, output_path: Path, dpi: int = 150) -> None:
    ...
    methods = ["tri_adaptive", "attention_v2", "attention", "gated"]
    labels_map = {
        "tri_adaptive": "Tri Adaptive\nFusion",
        "attention_v2": "Attention v2\nFusion",
        "attention": "Attention\nFusion",
        "gated": "Gated\nFusion",
    }
    colors = ["#1f77b4", "#16a085", "#2ecc71", "#9b59b6"]
...
def plot_per_subject_heatmap(summary: dict, output_path: Path, dpi: int = 150) -> None:
    ...
    dual_attn_v2 = summary.get("dual_branch", {}).get("attention_v2")
    if dual_attn_v2 and (dual_attn_v2.get("loso_per_subject") or dual_attn_v2.get("per_subject")):
        ps = dual_attn_v2.get("loso_per_subject") or dual_attn_v2.get("per_subject")
        model_rows.append(("DualBranch\n(Attention v2)", ps))

    tri_adapt = summary.get("dual_branch", {}).get("tri_adaptive")
    if tri_adapt and (tri_adapt.get("loso_per_subject") or tri_adapt.get("per_subject")):
        ps = tri_adapt.get("loso_per_subject") or tri_adapt.get("per_subject")
        model_rows.append(("TriBranch\n(Adaptive)", ps))
...
def plot_baseline_comparison(summary: dict, output_path: Path, dpi: int = 150) -> None:
    ...
    entries = [
        ...
        ("TriBranch\n(Adaptive)", summary.get("dual_branch", {}).get("tri_adaptive"), "#1f77b4"),
        (
            "DualBranch\n(Attention v2)",
            summary.get("dual_branch", {}).get("attention_v2"),
            "#16a085",
        ),
...
def main() -> None:
    ...
    if baseline_c:
        baseline_c["model"] = "Baseline C: CWT+EEG-ViT"
        log.info("  Loaded Baseline C (EEG-ViT-only)")
    ...
    dual_attn_v2 = load_dual_branch(
        _from_index(
            "dual_branch",
            "attention_v2_within_subject",
            rd / f"real_dual_branch_attention_v2_{bshort}.json",
        ),
        _from_index(
            "dual_branch",
            "attention_v2_loso",
            rd / f"real_dual_branch_attention_v2_{bshort}_loso.json",
        ),
    )
    if dual_attn_v2:
        log.info("  Loaded Dual-Branch (attention_v2)")

    tri_adapt = load_tri_branch(
        _from_index(
            "tri_branch",
            "tri_branch_adaptive_within_subject",
            rd / f"real_tri_branch_adaptive_{bshort}.json",
        ),
        _from_index(
            "tri_branch",
            "tri_branch_adaptive_loso",
            rd / f"real_tri_branch_adaptive_{bshort}_loso.json",
        ),
    )
    if tri_adapt:
        log.info("  Loaded Tri-Branch (adaptive)")
    ...
    summary = {
        ...
        "dual_branch": {
            "tri_adaptive": tri_adapt,
            "attention_v2": dual_attn_v2,
            "attention": dual_attn,
            "gated": dual_gated,
        },
    ...
    if not rows:
        ...
        for label, r in [
            ...
            ("TriBranch Adaptive", tri_adapt),
            ("DualBranch Attention v2", dual_attn_v2),
